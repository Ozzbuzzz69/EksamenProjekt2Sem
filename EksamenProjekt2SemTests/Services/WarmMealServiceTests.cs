...
namespace EksamenProjekt2Sem.Services.Tests
{
    ...
    public class WarmMealServiceTests
    {
        ...
        {
            // Arrange
            var database = new GenericDbService<WarmMeal>();
            var service = new WarmMealService(database);
            var item = new WarmMeal { Id = 3, Ingredients = "Burger", MeatType = "Beef", Price = 40.00, MinPersonAmount = 1, };
            ...
            using (var context = new FoodContext(_options))
            {
                var result = await context.WarmMeals.FindAsync(3);
