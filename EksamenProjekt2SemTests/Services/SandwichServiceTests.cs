...
namespace EksamenProjekt2Sem.Services.Tests
{
    ...
    public class SandwichServiceTests
    {
        ...
        {
            // Arrange
            var database = new GenericDbService<Sandwich>(); 
            var service = new SandwichService(database);
            var item = new Sandwich { Id = 3, Ingredients = "Turkey & Cheese", MeatType = "Turkey", Price = 19.95, Category = "Standard" };
            ...
            using (var context = new FoodContext(_options))
            {
                var result = await context.Sandwiches.FindAsync(3);

                Assert.IsNotNull(result, "Sandwich blev ikke fundet i databasen.");
                Assert.AreEqual("Ham & Cheese", result.Ingredients, "Sandwich med forkerte ingredienser fundet");
        ...
        [TestMethod]
        public void DeleteSandwich_InvalidId_ThrowsException()
        {
            // Arrange
            var service = new SandwichService(_dbService);

            // Act & Assert
            var ex = Assert.ThrowsException<Exception>(() => service.DeleteSandwich(999));
            Assert.AreEqual("Sandwich with id 999 not found.", ex.Message);
        }
        [TestMethod]
